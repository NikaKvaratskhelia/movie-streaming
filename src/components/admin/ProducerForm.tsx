...
type ProducerFormProps = {
  ...
  onAdd: (data: Partial<Producer>) => void;
  onUpdate: (id: number, data: Partial<Producer>) => void;
  mode: "edit" | "add";
  data?: Producer | null;
};
...
export default function ProducerForm({
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = (e) => {
    ...
    const payload: Partial<Producer> = {
    ...
    >
      ...
      >
        <div className="w-full flex justify-between items-center mb-3">
          <h3 className="text-2xl font-semibold">Producer</h3>
          <button
            type="button"
            onClick={() => onOpenChange(false)}
            aria-label="Close"
